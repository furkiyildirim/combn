...
def save_progress(db_name, last_index):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS progress (id INTEGER PRIMARY KEY, last_index INTEGER)''')
    c.execute('''INSERT OR REPLACE INTO progress (id, last_index) VALUES (?, ?)''', (1, last_index))
    conn.commit()
    conn.close()

def load_progress(db_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS progress (id INTEGER PRIMARY KEY, last_index INTEGER)''')
...
def generate_combinations(db_name, letters, length, start_index=0):
    if length == 0:
        return ['']
    smaller_combinations = generate_combinations(db_name, letters, length - 1)
    combinations = []
    line = 0
    for char in letters:
        for comb in smaller_combinations:
            ...
            save_progress(db_name, line)  # Save progress
    return combinations
...
def generate_and_save_combinations(config):
    ...
    start_index = load_progress(db_name)
    with Pool(cpu_count()) as pool:
        combination_data = pool.map(process_combination, generate_combinations(db_name, letters, length, start_index))
